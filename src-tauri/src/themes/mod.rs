//! User themes on disk: the folder, listing, saving and a live watcher that
//! tells the UI when files change so new/edited themes show up instantly.
...
/// Write a theme file into the themes folder. The name is sanitised to a
/// single path segment and given a `.json` extension if missing.
pub fn save_theme_file(app: &AppHandle, filename: &str, content: &str) -> std::io::Result<()> {
    let dir = themes_dir(app);
