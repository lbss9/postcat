...
import { PC_API } from "@/scripting/pcApi";

export type CodeLang = "javascript" | "json" | "html" | "xml" | "text";

export interface CodeEditorHandle {
  insert: (text: string) => void;
  /** collapse every foldable region (objects / arrays / blocks) */
  foldAll: () => void;
...
const highlight = HighlightStyle.define([
  ...
  { tag: [t.bool], color: "var(--j-bool, var(--j-num))" },
  { tag: [t.null], color: "var(--j-null, var(--j-num))" },
...
const CodeEditor = forwardRef<CodeEditorHandle, {
  ...
  indent?: string;
  /** soft-wrap long lines (toggled live, no remount) */
  wrap?: boolean;
  /** viewer mode: no edits, no cursor, no autocomplete */
  readOnly?: boolean;
  /** show the fold gutter so blocks / objects / arrays can be collapsed */
  foldable?: boolean;
}>(function CodeEditor(
  {
    value,
  ...
  useImperativeHandle(ref, () => ({
    ...
    foldAll() {
      if (view.current) foldAll(view.current);
    },
    unfoldAll() {
      if (view.current) unfoldAll(view.current);
    },
  }));
    ...
    const state = EditorState.create({
      ...
      extensions: [
        ...
        ...(readOnly
          ...
          : [closeBrackets(), autocompletion({ activateOnTyping: true, icons: true })]),
  ...
  // live wrap toggle — reconfigure the compartment instead of remounting
  useEffect(() => {
  ...
  return <div className={`code-editor${readOnly ? " cm-readonly" : ""}`} ref={host} />;
});
